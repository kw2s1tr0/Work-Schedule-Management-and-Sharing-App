...
public class PutRegularScheduleServiceImpl implements PutRegularScheduleService {
  ...
  public RegularScheduleInputRecord toRegularScheduleRecord(RegularSchedule regularSchedule) {
    ...
    // レギュラースケジュールの存在チェック
    regularScheduleService.existRegularSchedule(
        regularSchedule.getId(), regularSchedule.getUserId());

    // レギュラースケジュールの重複チェック
    regularScheduleService.checkRegularSchedule(
        regularSchedule.getId(),
        regularSchedule.getUserId(),
        regularSchedule.getStartDate(),
        regularSchedule.getEndDate(),
        regularSchedule);

    RegularScheduleInputRecord record =
        RegularScheduleInputRecord.builder()
