...
public class PutDefaultScheduleServiceImpl implements PutDefaultScheduleService {
  ...
  public DefaultScheduleInputRecord toDefaultScheduleRecord(
    ...
    // デフォルトスケジュールの重複チェック
    defaultScheduleService.checkDefaultSchedule(
        form.id(), userId, form.startDate(), form.endDate());

    DefaultScheduleInputRecord record =
        DefaultScheduleInputRecord.builder()
            .id(form.id())
            .userId(userId)
