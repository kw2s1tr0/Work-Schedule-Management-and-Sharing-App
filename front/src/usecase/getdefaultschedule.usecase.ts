...
export async function GetDefaultScheduleUsecase(
  ...
  const defaultscheduleResList: DefaultscheduleRes[] = await get(getSingleScheduleReq, type, cookie);
  const defaultScheduleDTOList: DefaultScheduleDTO[] = toDTO(defaultscheduleResList);
...
async function get(getSingleScheduleReq: GetSingleScheduleReq, type: ServerOrClientEnum, cookie?: string): Promise<DefaultscheduleRes[]> {
  ...
  const response = await fetcher(`/api/defaultSchedule?${params.toString()}`, MethodEnum.GET, type, undefined, cookie);
  ...
  const defaultscheduleResList: DefaultscheduleRes[] = data as DefaultscheduleRes[];

  return defaultscheduleResList;
}

function toDTO(defaultscheduleResList: DefaultscheduleRes[]): DefaultScheduleDTO[] {
  const defaultScheduleDTOList: DefaultScheduleDTO[] = defaultscheduleResList.map(
    (defaultscheduleRes) => {
