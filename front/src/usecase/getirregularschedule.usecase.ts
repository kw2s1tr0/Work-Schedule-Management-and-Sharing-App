...
export async function GetIrregularScheduleUsecase(
  ...
  const irregularscheduleResList: DefaultscheduleRes[] = await get(getSingleScheduleReq, type, cookie);
  const irregularScheduleDTOList: IrregularScheduleDTO[] = toDTO(irregularscheduleResList);
...
async function get(getSingleScheduleReq: GetSingleScheduleReq, type: ServerOrClientEnum, cookie?: string): Promise<DefaultscheduleRes[]> {
  ...
  const response = await fetcher(`/api/irregularSchedule?${params.toString()}`, MethodEnum.GET, type, undefined, cookie);
  ...
  const irregularscheduleResList: DefaultscheduleRes[] = data as DefaultscheduleRes[];

  return irregularscheduleResList;
}

function toDTO(irregularscheduleResList: DefaultscheduleRes[]): IrregularScheduleDTO[] {
  const irregularScheduleDTOList: IrregularScheduleDTO[] = irregularscheduleResList.map(
    (irregularscheduleRes) => {
