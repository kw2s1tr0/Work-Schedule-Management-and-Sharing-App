...
export async function DeleteIrregularScheduleUsecase(id: string, type: ServerOrClientEnum): Promise<string> {
  return await deleteSchedule(id, type);
}

async function deleteSchedule(id: string, type: ServerOrClientEnum): Promise<string> {
  const response = await fetcher(`/api/irregularSchedule/${id}`, MethodEnum.DELETE, type);
